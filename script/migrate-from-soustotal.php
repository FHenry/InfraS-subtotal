<?php
	...
	************************************************/
	...
	/************************************************
	* 	\file		./infrastructure/script/migrate-from-soustotal.php
	* 	\ingroup	InfraS
	* 	\brief		Wrapper CLI / web pour exécuter la migration des données
	*				soustotal (Iouston) → infrastructure et, optionnellement,
	*				désactiver + nettoyer le module soustotal.
	*
	*	Usage :
	*		- Simulation (par défaut)		: accès web (admin requis)
	*		- Exécution réelle				: ?confirm=yes
	*		- Exécution + cleanup soustotal	: ?confirm=yes&cleanup=yes
	...
	************************************************/
	...
	// Libraries ************************************
	dol_include_once('/infrastructure/core/lib/infrastructureMigrateSoustotal.lib.php');
	...
	call_user_func($logger, 'Migration soustotal → infrastructure — entity '.((int) $conf->entity).' — mode : '.($confirm ? 'EXECUTION' : 'SIMULATION (ajouter ?confirm=yes pour exécuter)'));
	call_user_func($logger, str_repeat('-', 80));

	$res	= infrastructure_migrateFromSoustotal($db, $conf, !$confirm, $logger);
	...
	if (! $res['success']) {
		...
		call_user_func($logger, 'Pour exécuter réellement : ajouter ?confirm=yes (+ ?cleanup=yes pour désactiver soustotal).');
	} else {
		call_user_func($logger, 'SUCCÈS : migration appliquée.');
		if ($cleanup) {
			call_user_func($logger, '');
			call_user_func($logger, 'Nettoyage du module soustotal...');
			$resClean	= infrastructure_cleanupSoustotal($db, $conf, $logger);
			if ($resClean) {
