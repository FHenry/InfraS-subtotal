...
-- 18.4.3 : rename constant INFRASTRUCTURE_MANAGE_COMPRIS_NONCOMPRIS → INFRASTRUCTURE_MANAGE_OL
-- (alignement avec la nouvelle terminologie plus générique : « lignes optionnelles » au lieu de « compris/non compris »)
-- Préserve la valeur en cas d'installation existante. Si la nouvelle constante existe déjà, supprime simplement l'ancienne.
DELETE FROM llx_const WHERE name = 'INFRASTRUCTURE_MANAGE_COMPRIS_NONCOMPRIS' AND EXISTS (SELECT 1 FROM (SELECT name FROM llx_const WHERE name = 'INFRASTRUCTURE_MANAGE_OL') AS t);
UPDATE llx_const SET name = 'INFRASTRUCTURE_MANAGE_OL' WHERE name = 'INFRASTRUCTURE_MANAGE_COMPRIS_NONCOMPRIS';

-- 18.4.3 : rename constant INFRASTRUCTURE_TFIELD_TO_KEEP_WITH_NC → INFRASTRUCTURE_COL_2_KEEP_WITH_OL
-- (alignement avec la nouvelle terminologie : « colonnes à conserver avec les lignes optionnelles » au lieu de « champs à conserver avec le non-compris »)
-- Préserve la valeur en cas d'installation existante. Si la nouvelle constante existe déjà, supprime simplement l'ancienne.
DELETE FROM llx_const WHERE name = 'INFRASTRUCTURE_TFIELD_TO_KEEP_WITH_NC' AND EXISTS (SELECT 1 FROM (SELECT name FROM llx_const WHERE name = 'INFRASTRUCTURE_COL_2_KEEP_WITH_OL') AS t);
UPDATE llx_const SET name = 'INFRASTRUCTURE_COL_2_KEEP_WITH_OL' WHERE name = 'INFRASTRUCTURE_TFIELD_TO_KEEP_WITH_NC';

-- 18.4.3 : rename constant INFRASTRUCTURE_NONCOMPRIS_UPDATE_PA_HT → INFRASTRUCTURE_OL_REDUCE_PA
-- (alignement avec la nouvelle terminologie : « réduit le prix d'achat pour les lignes optionnelles » au lieu de « met à jour le PA pour les lignes non-compris »)
-- Préserve la valeur en cas d'installation existante. Si la nouvelle constante existe déjà, supprime simplement l'ancienne.
DELETE FROM llx_const WHERE name = 'INFRASTRUCTURE_NONCOMPRIS_UPDATE_PA_HT' AND EXISTS (SELECT 1 FROM (SELECT name FROM llx_const WHERE name = 'INFRASTRUCTURE_OL_REDUCE_PA') AS t);
UPDATE llx_const SET name = 'INFRASTRUCTURE_OL_REDUCE_PA' WHERE name = 'INFRASTRUCTURE_NONCOMPRIS_UPDATE_PA_HT';