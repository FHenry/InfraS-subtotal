<?php
	...
	*   - cellule OL checkbox (si INFRASTRUCTURE_MANAGE_OL, titres uniquement)
	...
	************************************************/
	...
	if ($canSeeMargin && $user->hasRight('margins', 'creer') && $affectedByMarge > 0) {$colspan++;}																					// Colonne PA HT (prix de revient) — alignement avec $usemargins du core (objectline_title.tpl.php) qui ne rend la colonne que pour facture/facturerec/propal/commande
...
<!-- BEGIN PHP TEMPLATE infrastructureline_row_document.tpl.php -->
	...
	<?php } ?>
		...
		if (in_array($object->element, ['supplier_proposal', 'order_supplier', 'invoice_supplier'])) {
	...
	<?php
	// Cellule OL : checkbox pour les titres (qty 1-9), cellule vide pour les autres lignes
	// spéciales infrastructure (sous-totaux qty 91-99, textes libres qty 50) afin de préserver
	// l'alignement des colonnes avec la ligne <th>Opt</th> de l'en-tête principal.
	if ($object->statut == 0 && $createRight && !empty(getDolGlobalString('INFRASTRUCTURE_MANAGE_OL')) && $action != 'editline' && $action != 'selectlines') {
		if (TInfrastructure::isTitle($line)) {
			print '	<td class="infrastructure_ol">
						<input id="infrastructure_ol-'.$line->id.'" class="infrastructure_ol_chkbx" data-lineid="'.$line->id.'" type="checkbox" name="infrastructure_ol" value="1" '.(!empty($line->array_options['options_infrastructure_ol']) ? 'checked="checked"' : '').' />
					</td>';
		} else {
	...
	<?php
	// Cellule selectlines (mass actions)
	$Telement	= ['propal', 'commande', 'facture', 'supplier_proposal', 'order_supplier', 'invoice_supplier'];
	...
	$currentExtrafieldsConst	= isset($tableToConst[$tableElement]) ? getDolGlobalString($tableToConst[$tableElement]) : '';
	if (TInfrastructure::isTitle($line) && getDolGlobalInt('INFRASTRUCTURE_ALLOW_EXTRAFIELDS_ON_TITLE') && $currentExtrafieldsConst) {
		$extrafieldsline		= new ExtraFields($db);
		$extralabelsline		= $extrafieldsline->fetch_name_optionals_label($object->table_element_line);
		$selectedExtrafields	= explode(',', $currentExtrafieldsConst);
