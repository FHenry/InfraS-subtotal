	...
	class modInfrastructure extends DolibarrModules
	{
		...
		*/
		public function __construct($db)
		{
			...
			// Dictionnaries
			$this->dictionaries		= ['langs'				=> $this->name.'@'.$this->name,
											'tabname'			=> [MAIN_DB_PREFIX.'c_infrastructure_free_text'],			// List of tables we want to see into dictonnary editor
											'tablib'			=> [$langs->trans('InfrastructureFreeLineDictionary')],// Label of tables
											'tabsql'			=> ['SELECT f.rowid as rowid, f.label, f.content, f.entity, f.active FROM '. $db->prefix() .'c_infrastructure_free_text as f WHERE f.entity='.$conf->entity],	// Request to select fields
											'tabsqlsort'		=> ['label ASC'],											// Sort order
											'tabfield'			=> ['label,content'],										// List of fields (result of select to show dictionary)
											'tabfieldvalue'		=> ['label,content'],										// List of fields (list of fields to edit a record)
											'tabfieldinsert'	=> ['label,content,entity'],								// List of fields (list of fields for insert)
											'tabrowid'			=> ['rowid'],												// Name of columns with primary key (try to always name it 'rowid')
											'tabcond'			=> [isModEnabled($this->name)]
										];
			// Boxes
		...
		*/
		public function remove($options = '')
		{
			...
			$sql	= ['DELETE FROM '.$this->db->prefix().'const WHERE name like "INFRASTRUCTURE\_%" AND entity = "'.$conf->entity.'"',
